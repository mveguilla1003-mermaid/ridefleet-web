...
export async function buildMetadata({
  ...
  // One pre-rendered card per route per locale, generated by
  // `npm run build:og` with the SAME slug scheme — keep the two in sync.
  const slug = clean === '' ? 'home' : clean.slice(1).replace(/\//g, '-');
  const ogImage = `${site.url}/og/${locale}-${slug}.png`;
